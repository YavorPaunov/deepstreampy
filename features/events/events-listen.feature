...
Scenario: Event Listen
	...
	When the server sends the message E|A|L|eventPrefix/.*+

	# The client gets notified of new matching subscriptions
	And the server sends the message E|SP|eventPrefix/.*|eventPrefix/foundAMatch+
	...
	When the server sends the message E|SR|eventPrefix/.*|eventPrefix/foundAMatch+
	...
	When the server sends the message E|A|UL|eventPrefix/.*+
